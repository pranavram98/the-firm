...
def harvey_signoff(matter: Matter, round_dir: Path, draft: Path, *, cap_forced: bool = False) -> str:
    ...
    return header + cap_block + (
        ...
        "2. # Sign-off — formal ruling (accept residual [VERIFY] cites, note client risk).\n"
...
def mike_pack(matter: Matter, doc_list: str) -> str:
    ...
    return (
        ...
        "`final/pack/`). Edit each file **in place**; preserve structure, CSS classes, and "
        "`styles.css` link.\n\n"
        f"Documents:\n{doc_list}\n\n"
        "**Briefing Memo** — full matter brief (§1–§9 skeleton).\n"
        "**Argument Notes** — speaking-order numbered arguments with record basis.\n"
        "**Client Briefing (2pp)** — two A4 pages max; what the client must know Monday morning.\n"
        "**Reference Table** — every row cites a `sources/` file; no invented facts or cites.\n\n"
